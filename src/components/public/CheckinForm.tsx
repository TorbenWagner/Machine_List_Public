...
export function CheckinForm({ qrToken, people, onSuccess }: CheckinFormProps) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      const response = await fetch(`/api/public/machines/${qrToken}/checkin`, {
        method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      >
        {submitting ? uiTexts.publicMachine.submitting : uiTexts.publicMachine.submitCheckin}
      </button>
    </form>
  );
}
