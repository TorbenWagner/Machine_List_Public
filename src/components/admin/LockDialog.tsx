...
import { lockMachineAction, type MachineFormState } from "@/app/admin/(protected)/machines/actions";
...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? uiTexts.common.loading : uiTexts.admin.machines.actions.lock}
    </button>
  );
}

function LockModalContent({
  machineId,
  currentInformationText,
  onClose,
}: {
  machineId: string;
  currentInformationText: string | null;
  onClose: () => void;
}) {
  const action = lockMachineAction.bind(null, machineId);
    ...
    return (
      <div className="space-y-4">
        <p className="rounded-lg bg-green-50 px-4 py-3 text-sm text-green-800">
          {uiTexts.machineStatus.GESPERRT}
        </p>
        <button
          type="button"
          onClick={onClose}
  ...
  return (
    <form action={formAction} className="space-y-4">
      <p className="text-sm text-gray-600">{t.confirmLock}</p>
      <div>
        <label className="mb-1 block text-sm font-medium text-gray-700">
          {t.lockInfoLabel} <span className="text-gray-400">({uiTexts.common.optional})</span>
        </label>
        <textarea
          name="informationText"
          defaultValue={currentInformationText ?? ""}
          maxLength={2000}
          rows={3}
...
export function LockDialog({
  ...
  return (
    <>
      ...
      {open && (
        <Modal title={t.actions.lock} onClose={() => setOpen(false)}>
          <LockModalContent
            machineId={machineId}
            currentInformationText={currentInformationText}
